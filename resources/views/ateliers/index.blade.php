            ...
            @forelse($ateliers as $atelier)
                <div class="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 border border-gray-200">
                    ...
                    <div class="p-4">
                        ...
                        {{-- Note moyenne cliquable --}}
                        @php
                            $noteMoyenne = $atelier->noteMoyenne();
                            $nombreCommentaires = $atelier->nombreCommentaires();
                        @endphp
                        @if($noteMoyenne !== null)
                            <a href="{{ route('ateliers.commentaires', $atelier->getKey()) }}"
                               class="flex items-center gap-2 mb-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200 hover:bg-yellow-100 transition-colors group">
                                ...
                                <span class="text-sm font-semibold text-gray-700">{{ number_format($noteMoyenne, 1) }}</span>
                                <span class="text-xs text-gray-500">({{ $nombreCommentaires }} {{ $nombreCommentaires > 1 ? 'avis' : 'avis' }})</span>
                                <svg class="w-4 h-4 text-gray-400 ml-auto group-hover:text-gray-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if(!empty($atelier->salle) || !empty($atelier->salle_id))
                            <div class="mb-4 flex flex-col gap-2 text-sm bg-gray-50 rounded-lg p-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-gray-500">({{ $atelier->salle->capacite ?? '—' }} places)</span>
                                </div>

                                @if(!empty($atelier->salle->boutique))
                                    <div class="flex items-start gap-2 text-xs text-gray-600">
                                        <svg class="w-3 h-3 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <span class="font-medium">{{ $atelier->salle->boutique->nom }}</span>
